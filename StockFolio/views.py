...
from lib.alpha_advantage_retriever.equityretriever import execute_query_alpha
...
def portfolio(request):
    ...
    if request.method == 'POST':
        which_form = request.POST.get('which-form', '').strip()     # .strip() with no arguments (or None as the first argument) removes all whitespace at the start and end, including spaces, tabs, newlines and carriage returns.
        if which_form == 'find-stock':
            ...
            if symbol != '':
                ...
                stock_quote = execute_query_alpha(function="GLOBAL_QUOTE", symbol=symbol, )['Global Quote']
                print(stock_quote)
                return render(request, 'StockFolio/portfolio.html',
                              {'stock': stock_quote, 'portfolio': porfolio, 'portfolio_rows': plot(user_id), 'symbols': json.dumps(STOCK_SYMBOLS), 'money': money})
                # return render(request, 'StockFolio/portfolio.html',
        ...
        for idx, day in enumerate(days):
            ...
            for stock_index, stock in enumerate(data):
                if len(stock) <= idx:
                    continue
                if first:
                    row = {'Value': round(float(stock[idx]['Close']) * StockPortfolio.objects.filter(stock=stocks[stock_index].stock, user=user_id)[0].shares, 2), 'Date': day,
                           'Percent': (float(stock[idx]['Open']) - float(stock[idx]['Close'])) / float(stock[idx]['Close']) * 100, 'Volume': int(stock[idx]['Volume']), 'High': float(stock[idx]['High']), 'Low': float(stock[idx]['Low']),
                           'AdjClose': float(stock[idx]['AdjClose']), 'Open': float(stock[idx]['Open'])}
                    first = False
                else:
                    row['Date'] = day
